...
2. Check that the account has AI features enabled before starting generation. If not, explain that custom metric generation needs AI features enabled.
...
- `name` or explicit `metrics` (one is required)
- optional `description`
- optional `testUrls`
- optional explicit `metrics` (max 5)
- optional `htmlNeeded`
...
For existing work, use `analyze_list_custom_metric_generations` and then `analyze_get_custom_metric_generation`. Pass `verbose: true` only when the generated handler source is needed.

## Step 2: Generate and test

1. Call `analyze_request_custom_metric_generation`.
2. Tell the user it is async. Poll with `analyze_get_custom_metric_generation` only if they ask or the status is needed immediately.
3. Run tests with `analyze_run_custom_metric_generation_tests` when generation is in a valid state and test URLs exist.
...
Use `analyze_update_custom_metric_container_project` to change `enabled`, `containerParams`, `customJsResources`, or `customJsScripts` on an existing link. Use `analyze_unlink_custom_metric_container_from_project` to detach it from a project; confirm first because future crawls stop producing those metrics.

## Step 4: Delete

Use `analyze_delete_custom_metric_generation` only after explicit confirmation. It soft-deletes the generation and linked `CustomMetricContainer`; project crawls stop producing those metrics, but historical metric values remain.
